...
!*                                                                            *
!*   Following folks also contributed to update the code                      *
...
!_____________ Set Local Parameters
   ...
   ![OPTION] This value should be consist with the same variable defined in the start_mpi.f90
...
!    INTEGER,parameter:: SLOPETYP = 9   !★基底流出を起こさせない場合にActivate
!
! 1	       0-8
! 2	       8-30
! 3	       > 30
...
!Location data
   ...
   Integer,intent(IN):: SoilClass
   
   !TMP
   real,intent(IN):: CTI_dif !シミュレーション場所のCTI値－集水域平均CTI値
...
!Soil type, which determined various soil properties
   INTEGER SOILTYP  !Soil type index 1-9 (10, an original value)
...
!      Call crown_coverage   ()
!      
!      !★土を凍らせない条件の時にActivate
!      !SFLXALL_SRC_VER_2.7.1.f90にもmodificationする箇所あり
!      !Do i=1, NSOLD; SH2O(i) = SMC(i); Enddo
!      
!      SOILM = 0.0
!      do i = 1, NSOIL
!         SOILM = SOILM + SLDPTH(i)*SMC(i) !TOTAL SOIL COLUMN WATER CONTENT in M
...
!   !★新しい計算方法(テスト中)
!   !林床光量が林外の95.12%以下（LAI=0.1以上に相当）の林床セルを、植被部と定義
...
IF ( doy==Day_in_Year ) then
   ...
   do no=1, Max_no
      ...
      if ( Life_type(p)==7 .or. Life_type(p)==8 ) then
         ...
         sla_acclimated(no) = 250 * ( (12/TO_f(p))**(-0.43) )
         sla_acclimated(no) = sla_acclimated(no) /100/100
...
![Option] basal temperature gradually matches to the climate
IF ( doy==Day_in_Year ) then
