...
!est_capacity(1:PFT_no): establishment potential for each PFT based on climatic range
   ...
   do p = 1, PFT_no
      ...
      ![OPTION] For reducing sharpness of climatic envelope
      !y = 1.0
      y = 1.0 + 0.2 * (randf() - 0.5) !y has a uniform distribution at 0.9-1.1
      
      if ( p==C3g_no .or. p==C4g_no                  ) then; est_capacity(p)=.false. ;endif
      if ( y * tmp_coldest_20yr_ave >=   TC_max  (p)     ) then; est_capacity(p)=.false. ;endif
      if ( y * gdd_20yr_ave         >=   GDD_max (p)     ) then; est_capacity(p)=.false. ;endif
      if ( y * gdd_20yr_ave         <=   GDD_min (p)     ) then; est_capacity(p)=.false. ;endif
      if ( y * dry_month            >    DM_max  (p)     ) then; est_capacity(p)=.false. ;endif
!      if ( Life_type(p)==2 .and. dfl_fire>50*365      ) then; est_capacity(p)=.false. ;endif
   ...
   if (Est_scenario==5) then
      do p = 1, PFT_no
         est_capacity(p) = Est_pft_OnOff(p)
      enddo
   endif
...
Do j = 1, Dived
   ...
   do while ( tree_exist(new) )
      new = new + 1
      !if ( new > Max_no ) return !Mod 2020/Nov/17
      if ( new > Max_no ) exit    !Mod 2020/Nov/17
   end do
   if ( new > Max_no ) return     !Mod 2020/Nov/17
   ...
   !Acclimated leaf properties
   pn_f_acclimated (new) = PN_f (pft(new))
   sla_acclimated  (new) = SLA  (pft(new))
   pmax_acclimated (new) = Pmax (pft(new))
...
![OPTION] For fixing C3-type grass
...
!チシマザサ以外のササは常にOFF チシマザサだけにするときはこの部分を使うこと
!      pft_exist(C3g_no) = .false.
!      pft_exist(C4g_no) = .false.
!      pft_exist(23)     = .true.
!		 pft_exist(24)     = .false. 
...
!ミヤコザサ以外のササは常にOFF ミヤコザサだけにするときはこの部分を使うこと
!      pft_exist(C3g_no) = .false.
!      pft_exist(C4g_no) = .false.
!      pft_exist(23)     = .false.
! 		pft_exist(24)     = .false.
...
!_____________ Return when this site does not have satisfies minimum-fuel-load
   ...
   End do
   x = x + sum(gmass_leaf(:,:)) + sum(gmass_rod(:,:)) + sum(gmass_available(:,:))
...
!_____________ moist_factor calculation
   ...
   !mass_total(PFT_no): total above ground biomass for each PFT
      ...
      mass_sum(p) = sum(gmass_leaf(:,:)) + sum(gmass_rod(:,:)) + sum(gmass_available(:,:))
...
![OPTION] For causing fire at a regular interval
!  if ( mod( counter, 200*365 ) == 1 .or. counter==1) then
!     fire_prob = 1.0
!  else
!     fire_prob = 0.0
!  endif
   
   if (counter == counter_begin) then
      fire_prob = 1.0
   else
      fire_prob = 0.0
   endif
...
!_____________ When fire occurs
   ...
   mass_combust= mass_combust + sum(gmass_rod (:,:))      ; gmass_rod      (:,:)= 0.0
...
!_____________ Calculate potential fire intensity
   ...
   x = ( sum(gmass_leaf(:,:)) + sum(gmass_rod(:,:)) ) / DivedG/ DivedG 
   x = max(x , 0.001)                       !x: Average ABG of grass (gDM/grass_cell)
   do i=1, DivedG
   do j=1, DivedG
      grass_avandance(i,j) = ( gmass_leaf(i,j) + gmass_rod(i,j) )/ x
   end do
   end do
   ...
   do j=1, DivedG
      ...
      fuel_live = ( gmass_leaf(i,j) + gmass_rod(i,j) ) * Unit_conv2 + &
...
!   End do
   ...
   Do i=1, PFT_no
   if ( Life_type(i)==3 .or. Life_type(i)==4 .or. Life_type(i)==9 ) then
      phenology(i) = .false.
      dfl_leaf_shed(i) = 0
   end if
   End do
...
!  if ( height(no)-bole(no)<=2 .and. age(no)>3   ) mort_etc = 0.1
   if ( dbh_heartwood(no)+dbh_sapwood(no) > 1.00 ) mort_etc = mort_etc + 0.1
   if ( age(no) >= AGE_max(p)                    ) mort_etc = mort_etc + 0.1
...
!**************************************************************************************************
! Mass flowering of Sasa layer
!**************************************************************************************************
Subroutine sasa_bloom ()
...
!_____________ Mass flowing
   ...
   DO j=1, DivedG
      ...
      flux_litter_ag = flux_litter_ag + max(0.0, a1- 1.0)
      flux_litter_bg = flux_litter_bg + max(0.0, a2-50.0)
      ...
      gmass_stock    (i,j) = 50.0
      lai_grass      (i,j) = gmass_leaf(i,j) * SLA(C3g_no)
      
   END DO
   END DO
   
   !Reser Turnoff Phenology
   do p=1, PFT_no
      if (Life_type(p)==9) then  
         dfl_leaf_onset(p) = 0
         dfl_leaf_shed (p) = 0
