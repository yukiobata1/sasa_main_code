...
!_____________ Update of Ecosystem Total Carbon Pool running record
   ...
   z = sum(gmass_leaf (:,:)) + sum(gmass_rod      (:,:)) + sum(gmass_root (:,:)) + &
       sum(gmass_croot(:,:)) + sum(gmass_available(:,:)) + sum(gmass_stock(:,:))    
...
If (doy==150) then
   ...
   sumup(p) = &
   sum(gmass_leaf (:,:)) + sum(gmass_rod      (:,:)) + sum(gmass_root (:,:)) + &
   sum(gmass_croot(:,:)) + sum(gmass_available(:,:)) + sum(gmass_stock(:,:))    
