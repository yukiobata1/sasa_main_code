...
!*                                                                            *
!*   Following folks also contributed for code update                         *
!*        Dr. Tomo'omi KUMAHGAI (The University of Tokyo)                     *
!*        Dr. Taro NAKAI (National National Taiwan University)                *
!*                                                                            *
...
!*                                                                            *
!*   Latest information and code using policy are available at                *
!*   http://seib-dgvm.com/                                                    *
!******************************************************************************
...
!  INTEGER,parameter:: SLOPETYP = 9   ![OPTIONAL] Activate to disable bottom runoff
...
!Location data
   ...
   real,intent(IN):: CTI_dif !CTI@simulation site - CTI@grid average
...
!Parameter variables whose initial values will be given by the parameter file
   ...
   !Daily sumup�̂��߂ɍ������ǉ������ϐ�
   real:: ETP_sum     = 0.0 !���ݏ����U�� (W M-2��mm/day�ɕϊ�����)
   real:: EC_sum      = 0.0 !CANOPY WATER EVAPORATION  (����)
   real:: EDIR_sum    = 0.0 !DIRECT SOIL EVAPORATION   (����)
   real:: ETT_sum     = 0.0 !TOTAL PLANT TRANSPIRATION (����)
   real:: ESNOW_sum   = 0.0 !SUBLIMATION FROM SNOWPACK (����)
   real:: SNOMLT_sum  = 0.0 !SNOW MELT (����)
...
!Exchange soil properties when NOAH-module is activated
   INTEGER SOILTYP  !Soil type index 1-9 (10�ԂɓƎ��̒l����͂���)
...
if (Flag_output_write) then
   ...
   open ( File_no(16), file = 'output_npp.txt'          )
   open ( File_no(17), file = 'output_gpp.txt'          )
   open ( File_no(18), file = 'output_statwater.txt'    )
   open ( File_no(20), file = 'output_BasalArea.txt'    )
   open ( File_no(21), file = 'output_DBH_hist.txt'     )
endif
...
!Time counters2 (wild fire and phenology related)
   ...
   Do p=1, PFT_no
      dfl_leaf_onset(p) = dfl_leaf_onset(p) + 1 !Days from the last leaf onset
      dfl_leaf_shed (p) = dfl_leaf_shed (p) + 1 !Days from the last leaf shedding
   End do
...
!For adjusting CTI of the simualting point
...
!_____________ DAILY PSYSICAL PROCESSES
   ...
   !co2atm = aco2_annual(min(year_climate,YearMaxCO2)) !Progress
    co2atm = aco2_annual(140)                          ![OPTIONAL] Fixed at 1990
   !co2atm = aco2_annual(1)                            ![OPTIONAL] Fixed at 1850
...
!   do i=1, Dived
!   do j=1, Dived
!       if (par_floor_rel(i,j)<=0.951229) n=n+1 !LAI=0.1�ȏ�ɑ���
!   enddo
!   enddo
!   if (n<=Dived) then
...
!For preparing output variables
   !x: Number of Time step in a day
   x = real(TimeStepMax)
...
!This paragraph contains all procedures for providing variables from NOAH-LSM to SEIB-DGVM
   ...
   pool_snow_RunningRecord(2:Day_in_Year) = pool_snow_RunningRecord(1:Day_in_Year-1)
   ...
   pool_snow_RunningRecord(1) = pool_snow
...
IF ( doy==Day_in_Year ) then
   ...
   !Leaf functional trait parameters were given as functions of leaf longevity
   !This is only assumed for Cold-Temperate woody PFTs (CTeNE and CTeBS)
   do no=1, Max_no
      p = pft(no) ! p: PFT number
      
      !Update leaf functional trait parameters by considering light intensity of each tree
      !x: relative exposure received during growing season (%)
      x = 100.0 * par_rel_sum(no) / max(1, par_rel_count(no) )
      
      !Initialize relative exposure recorders
      par_rel_sum  (no) = 0.0
      par_rel_count(no) = 0
      
      !Update of leaf metabolic properties
      !Currently, this procedure is only conducted for Cold-Temperate PFTs.
      !This procedure is conducted once a year because the relative light environment
      !for each tree would also control these properties in upcoming updates.
      if ( tree_exist(no))                        then
      if ( Life_type(p)==7 .or. Life_type(p)==8 ) then
         
         !!SLA (m2 gDM-1). Koike et al. (2001) Tree Physiology 21: 951-958
         !sla_acclimated(no) = 0.035 - 0.025 * max(0.0, x - 10.0) / 90
         !
         !if (p==14 .or. p==15 ) then !Late  succession trees
         !      sla_acclimated(no) = min(0.035, max(0.025, sla_acclimated(no)))
         !else                        !Early succession trees
         !      sla_acclimated(no) = min(0.025, max(0.010, sla_acclimated(no)))
         !endif
         !
         !!PN_f (proportion of nitrogen @ leaf) (gN / gDM)
         !Shinano et al. (2001) Tree Physiology 21: 617-624
         !pn_f_acclimated (no) = 1.75 * sla_acclimated(no) + 0.00625
         !
         !!Pmax. Shinano et al. (2001) Tree Physiology 21: 617-624
         !pmax_acclimated (no) = 35.0* (sla_acclimated(no)*100*100-50) / &
         !                       (sla_acclimated(no)*100*100)
         
         !SLA (m2 gDM-1) Based on Reich et al. (1997) PNAS 94
         sla_acclimated(no) = 0.025 * ( (12/TO_f(p))**(-0.43) )
         
         !PN_f (nitrogen concentration @ leaf) (gN / gDM)
         !Based on Reich et al. (1997) PNAS 94
         pn_f_acclimated (no) = 0.037 * ( (12/TO_f(p))**(-0.33) )
         
         !Pmax. !Based on Reich et al. (1997) PNAS94
         pmax_acclimated (no) = 360 * ( (12/TO_f(p))**(-0.67) ) 
         pmax_acclimated (no) = pmax_acclimated (no) / 1000.0 / sla_acclimated(no)
...
!   If ( year>Simulation_year-10) then !during the last 10 years
    If ( year>0) then !during simulation years
      Call output_climate &
...
!  if (doy == Day_in_Year) then
   if (doy == 349) then !@ 1 Aug
      Call output_annual   (File_no(13))
      Call output_forest   (File_no(14))
      Call output_BasalArea(File_no(20))
      Call output_DBH_hist (File_no(21))
   endif
   
   !Annual output (At the middle of growing season in Northern Hemisphere)
...
if (Flag_output_write) then
   ...
   close (File_no(16))   !output_npp.txt
   close (File_no(17))   !output_gpp.txt
   close (File_no(18))   !output_statwater.txt
   close (File_no(20))   !output_BasalArea.txt
   close (File_no(21))   !output_DBH_hist.txt
endif
...
![OPTION] For Hokkaido Japan (See Sato et al., 2023 Ecol. Res. for detail)
      a1 = 0.0 !sum of Basal Area of Cold Temperate conifer (m2)
      a2 = 0.0 !sum of Basal Area of Cold Temperate conifer (m2)
      a4 = 0.0 !sum of tree biomass except fine root (gDM)
      do no=1, Max_no
      if (tree_exist(no)) then
         p = pft(no)
         x = 0.5 * (dbh_heartwood(no) + dbh_sapwood(no)) !radius at 1.3m (m)
         if (Life_type(p)==7) a1 = a1 + 3.141592 * x * x !Cold Temperate conifer
         if (Life_type(p)==8) a2 = a2 + 3.141592 * x * x !Cold Temperate broad-leaf summergreen
         
        !a4 = a4 + mass_trunk(no) + mass_leaf(no) + mass_stock(no) + mass_available(no)
         a4 = a4 + mass_trunk(no) 
      endif
      enddo
      out1 = a1 / max(a1+a2, 0.00001) !Fraction of conifer tree
      out2 = a4 * (C_in_drymass/1000/1000) * (100/Max_loc) * (100/Max_loc) !gDM/stand -> MgC/ha
