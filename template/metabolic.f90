...
!  if ( .not. phenology(p) ) cycle
   ...
   Do i = 1, MaxParClass !for each relative PAR intensity
      ...
      do loop = 0, 5
         ...
         if ( (abs(x)<0.0001) .or. (tmp_air<=Tmin(p)) .or. &     
              (tmp_air>=Tmax(p)) .or. (tmp_air<=0.0) )       then
            ce_tmp=0.0
         else
...
!              accroding to their PFT and mean daytime PAR
   ...
   DO no=1, Max_no
      ...
      !par_ave: Mean daytime PAR of each tree (Relative value: 0.0-1.0)
      par_ave = 0.0 
      do i = height(no), bole(no)+1, -1
         par_ave = par_ave  + x * par_direct_rel(no,i) + (1.0-x) * par_diffuse_rel(i)
      end do
      par_ave = par_ave  / (height(no)-bole(no))
      
      !daily sumup of light intensity for each tree crowns (Inserted on 2021/April/15)
      par_rel_sum  (no) = par_rel_sum  (no) + par_ave
      par_rel_count(no) = par_rel_count(no) + 1      
      
      !i: MaxParClass * relative par intensity of this tree (1~MaxParClass)
      i = int( real(MaxParClass) * par_ave )
      i = min(max(i,1),MaxParClass)
      
     !psat  (no) = psat_today  (p,i)                                   !Disabled 2021/April/15
      psat  (no) = psat_today  (p,i) * (pmax_acclimated(no) / Pmax(p)) !Mod 2021/April/15
...
!_____________ Herbaceous PFTs
   ...
   !Sasa extension
   do i=1, PFT_No; if (Life_type(i)==9 .and. pft_exist(i)) p=i; enddo
...
!Calculate daily maintenance cost
   ...
   !a2: maintenance respiration rate of fine root per unit leaf-mass (g DM g DM-1 day-1)
   !a2 = RM(p) * PN_r(p) * tmp_sensibility_soil / FR_ratio(p) !Previous method
   a2 = RM(p) * PN_r(p) * tmp_sensibility_soil * root_per_foliage
   
   !a3: turnover cost of fine root (g DM g DM-1 day-1)
   !a3 = 0.0
...
!ELSE
!   
!!For Farquer's method, still debagging
...
!TMP, Sasa extension, 草本layerは休眠しないと仮定
do p = 1, PFT_no
   if ( Life_type(P)==3 ) flag(p)=.false.
   if ( Life_type(P)==4 ) flag(p)=.false.
   if ( Life_type(P)==9 ) flag(p)=.false.
enddo
...
!  if ( (p.ne.C3g_no) .and. (p.ne.C4g_no) ) then
   if ( (p.ne.C3g_no) .and. (p.ne.C4g_no) .and. (Life_type(p).ne.9) ) then !Sasa extension
      do no=1, Max_no
...
DO no=1, Max_no
   ...
   !Exchanged  2021/April/15
!  mass_required_leaf  = RM(p)* PN_f(p)            * mass_leaf (no)          * tmp_sensibility_air
   mass_required_leaf  = RM(p)* pn_f_acclimated(no)* mass_leaf (no)          * tmp_sensibility_air
...
!  a1 = x * ( PN_f(p)*RM(p)*tmp_sensibility_air  + (TO_f(p)/Day_in_Year)*(RG_f(p)-RG_f_suck(p)) )
   a1 = x * ( pn_f_acclimated(no)*RM(p)*tmp_sensibility_air      &
              + (TO_f(p)/Day_in_Year)*(RG_f(p)-RG_f_suck(p))    )
...
do j=1, DivedG
   ...
   mass_required_ag = RM(p) * PN_f(p) * tmp_sensibility_air  * gmass_leaf(i,j) + &
                      RM(p) * PN_s(p) * tmp_sensibility_air  * gmass_rod (i,j)
	mass_required_bg = RM(p) * PN_s(p) * tmp_sensibility_soil * gmass_root(i,j)  
!↑crootrootの維持呼吸を切った(crootrootの維持呼吸を考慮する場合は下を使用)
!   mass_required_bg = RM(p) * PN_s(p) * tmp_sensibility_soil * gmass_root(i,j)  + &
!                      RM(p) * PN_r(p) * tmp_sensibility_soil * gmass_croot(i,j)   
   ...
   If (mass_required > 0.001) then
      if (gmass_leaf(i,j) > 1.0) then
         a1 = Frac_organ_remove * max(gmass_leaf(i,j), 1.0)
...
!      if (gmass_rod(i,j) >=0.0) then
!         a2 = Frac_organ_remove * gmass_rod(i,j)
!         gmass_rod(i,j)  = gmass_rod(i,j)  - a2
!        flux_litter_ag  = flux_litter_ag  + a2
!      endif
      
      if (gmass_root(i,j) >=0.0) then
         a3 = Frac_organ_remove * gmass_root(i,j)
         gmass_root(i,j)  = gmass_root(i,j)  - a3
         flux_litter_bg   = flux_litter_bg   + a3
      endif
      !追加!gmass_crootをターンオーバーさせるときは下を使用
      if (gmass_croot(i,j) >=0.0) then
!         a3 = Frac_organ_remove  * gmass_croot(i,j)
			a3 = 0.001 * gmass_croot(i,j)
         gmass_croot(i,j) = gmass_croot(i,j)  - a3
         flux_litter_bg   = flux_litter_bg   + a3
      endif
   End if
end do
...
if (tree_exist(no)) then
   ...
   !Inserted on 2021 APril 19 by Hisashi SATO
   if (dfl_leaf_onset(p) < Day_in_Year / TO_f(p)) then
      t_rate = 0.0
   else
...
!_____________ Herbaceous PFTs
   ...
   !turnover rates
   t_rate_leaf = TO_f(p)/Day_in_Year !for leaf      biomass
   t_rate_rod  = TO_s(p)/Day_in_Year !for rod       biomass
   t_rate_root = TO_r(p)/Day_in_Year !for fine root biomass
!実験的に粗根も回転させる
TO_c = 0.05
t_rate_croot = TO_c/Day_in_Year !for croot root biomass
   
!   !TMP Modifier for turnover rates
!   if (dfl_leaf_onset(p) < Day_in_Year/TO_f(p)) then
!      t_rate_leaf = 0.0
!      t_rate_root = 0.0
!   endif
   ...
   DO j=1, DivedG
      ...
      if ( gmass_leaf(i,j) >= 1.0 ) then
!     if ( gmass_leaf(i,j) >= 0.0 ) then
         t_mass = gmass_leaf(i,j) * t_rate_leaf  !leaf mass to drop (g/cell)
      ...
      !rod turnover
      if ( gmass_rod(i,j) >= 0.0 ) then
         t_mass = gmass_rod(i,j) * t_rate_rod  !rod mass to drop (g/cell)
         
         gmass_rod      (i,j) = gmass_rod      (i,j) - t_mass
         gmass_available(i,j) = gmass_available(i,j) + t_mass * RG_f_suck(p)
      ...
      !root turnover
      if ( gmass_root(i,j) >= 0.0 ) then
         t_mass          = gmass_root(i,j) * t_rate_root  !root mass to drop (g/m2)
...
!croot turnover(追加)
	      if ( gmass_croot(i,j) >= 0.0 ) then
         t_mass          = gmass_croot(i,j) * t_rate_croot  !root mass to drop !(g/m2)
         
         gmass_croot(i,j) = gmass_croot(i,j) - t_mass
         flux_litter_bg  = flux_litter_bg  + t_mass
      endif
      
      !Note: Coarse root turnover does not happen
...
!      id_location(no) = min(id_location(no), (int(0.99999+Max_loc/20.0))**2) !Addition: 2020Nov17
!      
...
!      a2 = 14.8 * ( (100*dbh_sapwood(no)+100*dbh_heartwood(no))**1.68 ) !Kajimoto et al (2006) 
    ...
    !Stop new leafing if season seems to go to bad.
    a1 = sum(lai_RunningRecord(1: 7,p))
    a2 = sum(lai_RunningRecord(8:14,p))
    if (a1>0.0 .and. a2>0.0) then
       a3 = sum(gpp_RunningRecord(1: 7,p))
       a4 = sum(gpp_RunningRecord(8:14,p))
       
       if (a3/a1 < a4/a2) a6 = 0.0
    endif
...
!Local variables
   ...
   real    mass_combust              !biomass combust (g dm/day/stand)
   real    x, a1, a2, a3, a4, a5, a6 !for temporal usage
...
!追加!一日に増える葉の制限をかけた
    ...
    !coarse root growth
       !a1: maximum coarse root mass requirement (g DM/cell)
        a1 = gmass_rod(i,j)*CrootRod_ratio(p) - gmass_croot(i,j)
        a1 = max(a1, 0.0)
       !a2: maximum increament of biomass that available resource permits (g/cell)
        a2 = gmass_available(i,j) / RG_s(p)
       !a3: actual increament of coarse root mass (g DM/cell)
        a3 = min(a1, a2)
       
       !foliage mass increament procedure
        gmass_available(i,j) = gmass_available(i,j) - a3 * RG_s(p)
        gmass_croot    (i,j) = gmass_croot    (i,j) + a3          
        
        resp_grass_bg   = resp_grass_bg   + a3 * (RG_s(p)-1.0)
        mass_combust    = mass_combust    + a3 * (RG_s(p)-1.0)
        npp (p)         = npp (p)         - a3 * (RG_s(p)-1.0)
		
    !ササの場合: excess resource will be used to rod growth
    If (Life_type(p)==9) then
       !a1: actual increament of rod mass (g DM/cell)
		a1 = gmass_available(i,j) / RG_s(p)
        
		gmass_rod (i,j)  = gmass_rod  (i,j)  + a1
        
        mass_combust = mass_combust + a1*(RG_s(p)-1.0)
        npp (p)      = npp (p)      - a1*(RG_s(p)-1.0)
		gmass_available(i,j)= 0.000000
      
    !草本の場合: reproduction: excess resource will be added to litter
    Else
...
DO i=1, DivedG
DO j=1, DivedG
   x = gmass_leaf (i,j) + gmass_rod      (i,j) + gmass_root (i,j) + &
       gmass_croot(i,j) + gmass_available(i,j) + gmass_stock(i,j)    
...
if ( height(no)-bole(no) <= Crown_depth_min ) cycle
   ...
   Case (1)     !Trees of tropical rain forest
      ...
      x = ALM_crown3(p) * hgt        !new crown height (m)
   ...
   Case (7,8)     !Cold temperate PFTs (Tatsumi et al. 2012 FORMATH vol11)
      x = height(no) * Step + 1.3              !Tree height (m)
      x = ALM_crown6(p) * (x ** ALM_crown7(p)) !New crown height (m)
      i = int( height(no) - x/STEP )           !new bole height (STEP)
      count = max(0, i - bole(no))
      
   Case default !default standard
      do i = 1, min(10, height(no)-bole(no)-1)
         if ( sum(npp_crownbottom(no,1:i))/i < npp_crowntop(no)*ALM_crown2(p) ) count = i
      end do
...
!     x = PI * ((46.4587*diameter_new)**2)
   ...
   elseif (Life_type(p)==7 .or. Life_type(p)==8)    then
      !Cold temperate PFTs (Tatsumi et al. 2012 FORMATH vol11)
      x = ALM_crown4(p) * ((diameter_new*100.0)**ALM_crown5(p))   !crown radius (m)
      x = PI * x * x
   else
      !other trees
      x = ALM_crown1(p) * (diameter_new**1.6)
   endif
   
   x = min( x, PI*CD_max(p)*CD_max(p)/4.0           ) !Consider maximum crown size
   x = max( x, 0.001                                ) !Consider minimum crown size
   x = min( x, PI*radius_limit(no)*radius_limit(no) ) !Consider neighboring crowns
   x = min( x, crown_area(no) * 1.5                 ) !Consider maximum growht rate
...
!     !SEIB Ver.Farquhar
!     a           = (x-Stat_water_min(p)) / abs(Stat_water_opt(p)-Stat_water_min(p))
!     b           = (Stat_water_max(p)-x) / abs(Stat_water_opt(p)-Stat_water_max(p))
